...
def is_clear(boat):
    ...
    """
    distances, angles = boat.get_lidar_data()
    control = boat.obs_avoider_controller.compute_control((distances, angles))
    boolean = control is None
    print(boolean)
    return boolean
...
def avoid_obstacle(boat):
    ...
    while not is_clear(boat):
        control = boat.obs_avoider_controller.compute_control(boat.get_lidar_data())
        if control is not None:
            boat.set_thruster_mode(control[0], control[1])
    ...
    with boat:
        ...
        while index < len(waypoints):
            ...
            if hlp.are_coordinates_close(
                boat.get_gps_coordinates(), desired_coordinates, tolerance_meters=5.0
            ):
